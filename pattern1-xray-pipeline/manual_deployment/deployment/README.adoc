...
Monitor the deployment in the OpenShift console under the Severless->Service menu (don't confuse it with Service under Netwoking!). It must reach the 4/4 or 3/4 state (last one meanining it's ready but scaled down to zero as there is nothing to process).
