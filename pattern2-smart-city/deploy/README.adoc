...
* When any vehichel enters the ULEZ, a certain fee (aka toll fee) must be applied to that vehicle
...
[source,bash]
----
oc rsync superset/config/superset-datasources.yaml $(oc get pod | grep superset- | awk '{print $1}'):/tmp
----
