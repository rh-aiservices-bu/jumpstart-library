...
## Generating numpy formatted image
def load_image(file_path):
    width = 300
    height = 300
    img = cv2.imread(file_path)  # BGR color format, shape HWC
    img = cv2.resize(img, (width, height))
    img = img.transpose(2,0,1).reshape(1,3,width, height)
...
parser.add_argument('--image_path', required=False, help='Absolute path of the image file')
parser.add_argument('--images_numpy_path', required=False, help='numpy in shape [n,w,h,c] or [n,c,h,w]')
...
# imgs = cv2.imread(args['image_path'])
# initial_w = imgs.shape[1]
# initial_h = imgs.shape[0]
# n, c, h, w = 1, 3 ,300 300
# in_frame = cv2.resize(imgs, (w, h))
...
#img = tf.keras.preprocessing.image.load_img(args['image_path'])
...
#imgs = np.load(args['image_path'], mmap_mode='r', allow_pickle=False)

imgs = load_image(args['image_path'])
print(imgs)

#imgs = np.load(imgs, mmap_mode='r', allow_pickle=False)
